...
//		for (Entry<Integer, List<CRType>> rpyGroup: mapRPY_CRs.entrySet()) {
		mapRPY_CRs.entrySet().stream().parallel().forEach(rpyGroup -> {	
		...
		mapRPY_CRs.entrySet().stream().parallel().forEach(rpyGroup -> {	
			...
			final int rpy = rpyGroup.getKey();
			final int rpyIdx = rpy - rangeRPY[0];
			
			int seqStart = Math.max(rpy, rangePY[0]);
			int seqEnd = rangePY[1];
			
			if (seqEnd<seqStart) return; // continue;
			
			int[][] CR_PY_SumPub = new int[rpyGroup.getValue().size()][rangePY[1]-rangePY[0]+1];
//			char[][] mapCRID_Sequence = new char[mapRPY_CRs.get(rpy).size()][];
//			int[][] mapCRID_Type = new int[mapRPY_CRs.get(rpy).size()][];
			
			for (int crIdx=0; crIdx<rpyGroup.getValue().size(); crIdx++) {
				CRType cr = rpyGroup.getValue().get(crIdx);
				int x = crIdx;
